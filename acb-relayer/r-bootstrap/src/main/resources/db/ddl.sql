...
(
    ...
    KEY `idx_sender_and_state` (`sender_blockchain_product`, `sender_instance`, `process_state`),
    KEY `idx_amsq_idempotent_info` (`sender_domain_name`, `sender_identity`, `receiver_domain_name`,
                                    `receiver_identity`, `nonce`, `msg_sequence`),
...
(
    ...
    `gmt_modified`                datetime            DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `sdp_archive_auth_msg_id_idx` (`auth_msg_id`)
) ENGINE = InnoDB
  ROW_FORMAT = DYNAMIC;
...
(
    `id`                        INT(11) PRIMARY KEY       NOT NULL AUTO_INCREMENT,
    `service_id`                VARCHAR(64) BINARY UNIQUE NOT NULL,
    `type`                      VARCHAR(32)               NOT NULL,
...
CREATE INDEX ptc_service_state_idx
    ON ptc_service (state);
...
CREATE UNIQUE INDEX bta_domain_version
...
(
    ...
    `blockchain_product`        VARCHAR(64) BINARY DEFAULT NULL,
    `blockchain_id`             VARCHAR(128)       DEFAULT NULL,
    `ptc_service_id`            VARCHAR(64) BINARY DEFAULT NULL,
    `version`                   INT(11)            DEFAULT NULL,
...
(
    `id`             INT(11) PRIMARY KEY NOT NULL AUTO_INCREMENT,
    `ptc_service_id` VARCHAR(64) BINARY  NOT NULL,
    `owner_id_hex`   VARCHAR(255)        NOT NULL,
...
CREATE UNIQUE INDEX ptc_va_uidx_ptc_service_id_version_num
    ON ptc_verify_anchor (ptc_service_id, version_num);
CREATE UNIQUE INDEX ptc_va_uidx_owner_id_hex_version_num
    ON ptc_verify_anchor (owner_id_hex, version_num);
...
(
    ...
    `domain`             VARCHAR(128) BINARY NOT NULL,
    `ptc_service_id`     VARCHAR(64) BINARY  NOT NULL,
    `tpbta_lane_key`     VARCHAR(255) BINARY NOT NULL,
    `height`             BIGINT UNSIGNED     NOT NULL,
    `hash`               VARCHAR(66) BINARY  NOT NULL,
    `parent_hash`        VARCHAR(66) BINARY  NOT NULL,
    `state_timestamp`    TIMESTAMP(3)        NOT NULL,
    `raw_vcs`            LONGBLOB,
    `gmt_create`         DATETIME           DEFAULT CURRENT_TIMESTAMP,
    `gmt_modified`       DATETIME           DEFAULT CURRENT_TIMESTAMP
);
CREATE UNIQUE INDEX vcs_uidx_domain_height
    ON validated_consensus_state (ptc_service_id, domain, tpbta_lane_key, height);
CREATE UNIQUE INDEX vcs_uidx_domain_hash
    ON validated_consensus_state (ptc_service_id, domain, tpbta_lane_key, hash);
