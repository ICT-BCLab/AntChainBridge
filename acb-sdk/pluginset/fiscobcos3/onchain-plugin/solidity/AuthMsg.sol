...
contract AuthMsg is IAuthMessage, Ownable {
    ...
    // monitor contract address — for reference/audit; not used for access control in AM
    ...
    function setMonitorContract(address newMonitorAddress) external onlyOwner {
        require(newMonitorAddress != address(0), "AuthMsg: invalid monitor contract");
    ...
    function recvFromProtocol(address senderID, bytes message) external onlySubProtocols {
        ...
        bytes memory pkg = AMLib.encodeAuthMessage(amV1);
        emit SendAuthMessage(Utils.bytesToHexString(pkg));
