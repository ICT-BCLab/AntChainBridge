...
contract SDPMsg is ISDPMessage, Ownable {
    ...
    modifier onlyMonitorOrLegacySender() {
        require(
            monitorAddress == address(0) || monitorAddress == msg.sender,
    ...
    function sendMessage(string receiverDomain, bytes32 receiverID, bytes message) external onlyMonitorOrLegacySender {
        _sendMessage(receiverDomain, receiverID, msg.sender, message);
    }

    function sendMessage(string receiverDomain, bytes32 receiverID, address senderID, bytes message) external onlyMonitorOrLegacySender {
        _sendMessage(receiverDomain, receiverID, senderID, message);
    }

    function _sendMessage(string receiverDomain, bytes32 receiverID, address senderID, bytes message) internal {
    ...
    function sendUnorderedMessage(string receiverDomain, bytes32 receiverID, bytes message) external onlyMonitorOrLegacySender {
        _sendUnorderedMessage(receiverDomain, receiverID, msg.sender, message);
    }

    function sendUnorderedMessage(string receiverDomain, bytes32 receiverID, address senderID, bytes message) external onlyMonitorOrLegacySender {
        _sendUnorderedMessage(receiverDomain, receiverID, senderID, message);
    }

    function _sendUnorderedMessage(string receiverDomain, bytes32 receiverID, address senderID, bytes message) internal {
    ...
    function sendMessageV2(string receiverDomain, bytes32 receiverID, bool atomic, bytes message) external onlyMonitorOrLegacySender returns (bytes32) {
        return _sendMessageV2(receiverDomain, receiverID, msg.sender, atomic, message);
    }

    function sendMessageV2(string receiverDomain, bytes32 receiverID, address senderID, bool atomic, bytes message) external onlyMonitorOrLegacySender returns (bytes32) {
        return _sendMessageV2(receiverDomain, receiverID, senderID, atomic, message);
    }

    function _sendMessageV2(string receiverDomain, bytes32 receiverID, address senderID, bool atomic, bytes message) internal returns (bytes32) {
        _beforeSend(receiverDomain, receiverID, message);
    ...
    function sendUnorderedMessageV2(string receiverDomain, bytes32 receiverID, bool atomic, bytes message) external onlyMonitorOrLegacySender returns (bytes32) {
        return _sendUnorderedMessageV2(receiverDomain, receiverID, msg.sender, atomic, message);
    }

    function sendUnorderedMessageV2(string receiverDomain, bytes32 receiverID, address senderID, bool atomic, bytes message) external onlyMonitorOrLegacySender returns (bytes32) {
        return _sendUnorderedMessageV2(receiverDomain, receiverID, senderID, atomic, message);
    }

    function _sendUnorderedMessageV2(string receiverDomain, bytes32 receiverID, address senderID, bool atomic, bytes message) internal returns (bytes32) {
        _beforeSendUnordered(receiverDomain, receiverID, message);
        ...
        SDPLib.SDPMessageV2 memory sdpMessage = SDPLib.SDPMessageV2(
            {
                ...
                receiver: receiverID,
                atomicFlag: atomic ? SDP_V2_ATOMIC_FLAG_ATOMIC_REQUEST : SDP_V2_ATOMIC_FLAG_NONE_ATOMIC,
                nonce: _getAndUpdateSendNonce(receiverDomain, senderID, receiverID),
                sequence: UNORDERED_SEQUENCE,
                message: message,
                errorMsg: ""
            }
