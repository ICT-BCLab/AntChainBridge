...
public class FISCOBCOSBBCServiceTest {
    public static final String abiFile = getResourcePath("sdk/abi", "sdk/bin");

    public static final String abiFileSM = getResourcePath("sdk/abi/sm", "sdk/bin/sm");

    public static final String binFile = getResourcePath("sdk/bin", null);

    public static final String binFileSM = getResourcePath("sdk/bin/sm", null);
    ...
    private static String getResourcePath(String path, String fallbackPath) {
        URL resource = FISCOBCOSBBCService.class.getClassLoader().getResource(path);
        if (resource != null) {
            return resource.getPath();
        }
        if (fallbackPath != null) {
            return getResourcePath(fallbackPath, null);
        }
    ...
    public void testReadConsensusState() {
        ...
        BigInteger height = BigInteger.valueOf(fiscobcosBBCService.queryLatestHeight());
        ConsensusState consensusState = fiscobcosBBCService.readConsensusState(height);

        Assert.assertNotNull(consensusState);
        Assert.assertEquals(height, consensusState.getHeight());
        ...
        Assert.assertTrue(new String(consensusState.getStateData()).contains("transactionsRoot"));
        Assert.assertTrue(new String(consensusState.getStateData()).contains("receiptsRoot"));
        Assert.assertTrue(new String(consensusState.getStateData()).contains("stateRoot"));
    ...
    public void testSetupMonitorContract() {
        ...
        fiscobcosBBCService.setupAuthMessageContract();
        fiscobcosBBCService.setupSDPMessageContract();
        fiscobcosBBCService.setupMonitorContract();

        MonitorContract monitorContract = fiscobcosBBCService.getContext().getMonitorContract();
        Assert.assertNotNull(monitorContract);
        Assert.assertNotNull(monitorContract.getContractAddress());
        Assert.assertEquals(ContractStatusEnum.CONTRACT_DEPLOYED, monitorContract.getStatus());

        fiscobcosBBCService.setMonitorControl(2);
        Assert.assertEquals(ContractStatusEnum.CONTRACT_READY, monitorContract.getStatus());
    ...
    public void testReadCrossChainMessagesByHeight_sendUnordered() throws Exception {
        ...
        AssembleTransactionProcessor transactionProcessor = getTestContractProcessor();
        String senderContractAddress;
        try {
            senderContractAddress = transactionProcessor.deployByContractLoader(
                    "SenderContract", ListUtil.empty()).getContractAddress();
        } catch (Exception e) {
            throw new RuntimeException("failed to deploy sender contract", e);
        }
        System.out.println("sender contract:" + senderContractAddress);

        // 1. set sdp addr
        TransactionReceipt receipt = transactionProcessor.sendTransactionAndGetReceiptByContractLoader(
                "SenderContract",
                senderContractAddress,
                "setSdpMSGAddress",
                ListUtil.of(fiscobcosBBCService.getBbcContext().getSdpContract().getContractAddress()));
        ...
        try {
            ...
            transactionProcessor.sendTransactionAndGetReceiptByContractLoaderAsync(
                    ...
                    senderContractAddress,  // contract address
                    "sendUnordered", // function name
        ...
        CrossChainMessage message = messageList.get(0);
        Assert.assertEquals(CrossChainMessage.CrossChainMessageType.AUTH_MSG, message.getType());
        Assert.assertTrue(message.getProvableData().getLedgerData().length > 0);
        Assert.assertTrue(new String(message.getProvableData().getLedgerData()).contains("log"));
        Assert.assertTrue(message.getProvableData().getProof().length > 0);
        Assert.assertTrue(new String(message.getProvableData().getProof()).contains("receiptsRoot"));
        Assert.assertArrayEquals(
                message.getMessage(),
                new FISCOBCOS3HCDVSService().parseMessageFromLedgerData(message.getProvableData().getLedgerData())
        );

        System.out.println(HexUtil.encodeHexStr(message.getMessage()));
        IAuthMessage authMsg = AuthMessageFactory.createAuthMessage(message.getMessage());
    ...
    @Test
    public void testOffChainCommitteePtcCompatibility() {
        AbstractBBCContext mockValidCtx = mockValidCtx();
        fiscobcosBBCService.startup(mockValidCtx);

        fiscobcosBBCService.setupPTCContract();
        Assert.assertNotNull(fiscobcosBBCService.getContext().getPtcContract());
        Assert.assertEquals(ContractStatusEnum.CONTRACT_READY, fiscobcosBBCService.getContext().getPtcContract().getStatus());

        CrossChainLane lane = new CrossChainLane(
                new CrossChainDomain("fisco.web3.monitor.01"),
                new CrossChainDomain("fisco.web3.monitor.02")
        );
        Assert.assertTrue(fiscobcosBBCService.hasTpBta(lane, 1));
        Assert.assertTrue(fiscobcosBBCService.getSupportedPTCType().contains(PTCTypeEnum.COMMITTEE));

        fiscobcosBBCService.setPtcContract("committee-ptc-offchain-test");
        Assert.assertEquals("committee-ptc-offchain-test", fiscobcosBBCService.getContext().getPtcContract().getContractAddress());
    ...
    private AssembleTransactionProcessor getTestContractProcessor() {
        try {
            ...
            throw new RuntimeException("failed to create test contract processor", e);
        }
    }
