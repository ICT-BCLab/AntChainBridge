 ...
 */
package com.alipay.antchain.bridge.plugins.ethereum3.core.eth;
...
public class EthLog {
    ...
    private final ImmutableList<EthLogTopic> topics;

    /**
     * Instantiates a new EthLog.
     *
     ...
     */
    public static EthLog readFrom(final RLPInput in) {
     ...
     * @return the read log entry.
     */
    public static EthLog readFrom(final RLPInput in, final boolean compacted) {
        in.enterList();
        ...
        final Bytes data;
        if (compacted) {
            topics = in.readList(listIn -> EthLogTopic.wrap(Bytes32.wrap(readTrimmedData(in))));
            data = Bytes.wrap(readTrimmedData(in));
        } else {
            topics = in.readList(listIn -> EthLogTopic.wrap(listIn.readBytes32()));
            data = in.readBytes();
        }

        in.leaveList();
        return new EthLog(logger, data, topics);
    }
     ...
     * @return the topics
     */
    public List<EthLogTopic> getTopics() {
        return topics;
    }

    @Override
    public boolean equals(final Object other) {
        if (!(other instanceof EthLog that)) return false;
