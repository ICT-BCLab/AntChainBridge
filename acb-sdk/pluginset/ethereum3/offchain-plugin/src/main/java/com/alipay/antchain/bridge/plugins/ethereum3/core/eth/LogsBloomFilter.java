...
public class LogsBloomFilter extends DelegatingBytes {
  ...
  public static final class Builder {
     ...
     * @return the builder
     */
    public Builder insertLog(final EthLog log) {
      insertBytes((Bytes) log.getLogger());

      for (final EthLogTopic topic : log.getTopics()) {
        insertBytes(topic);
      }
      return this;
    }
     ...
     * @return the builder
     */
    public Builder insertLogs(final Collection<EthLog> logs) {
    ...
    public Builder insertBytes(final Bytes value) {
      setBits(Bytes.wrap(Hash.sha3(value.toArray())));
      return this;
    }
