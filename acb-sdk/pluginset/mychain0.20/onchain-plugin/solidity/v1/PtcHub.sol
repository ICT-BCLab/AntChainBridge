...
contract PtcHub is IPtcHub, Ownable {
    ...
    mapping(PTCTypeEnum => identity) public verifierMap;
    ...
    function addTpBta(bytes calldata rawTpBta) external override onlyOwner() {
        ...
        identity verifier = verifierMap[AcbCommons.decodePTCCredentialSubjectFrom(ptr.ptcCrossChainCert.credentialSubject).ptcType];
        require(verifier != identity(0x0), "no ptc veifier set");
    ...
    function addPtcVerifier(identity verifierContract) external override onlyOwner() {
        PTCTypeEnum ptcType = IPtcVerifier(verifierContract).myPtcType();

        require(verifierMap[ptcType] == identity(0x0), "ptc verifier already exists");
    ...
    function removePtcVerifier(PTCTypeEnum ptcType) external override onlyOwner() {
        require(verifierMap[ptcType] != identity(0x0), "ptc verifier not exists");
