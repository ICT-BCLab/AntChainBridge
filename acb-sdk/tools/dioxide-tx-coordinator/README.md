...
## 已确认过期的预留缺口（2026-09-06）

`dx.isn` 不是永久单调的“已分配序号”。节点丢弃未确认交易后，账户可能回到最后已归档用户交易的下一序号。
只保持本地高水位、继续发送更大序号，会产生无法执行的缺口。本轮新增显式过期预留流程，历史业务不重发。

- 运维工具 `reclaim_expired.py` 默认 dry-run。指定账户、连续范围和预期 `next_isn`，核对原网络 checkpoint、
  已归档用户交易边界、节点当前 ISN、每一份原签名的时间和 TTL，以及节点明确的 10005。
- 过期证据使用已提交链头之前 1000 块的区块哈希和链上时间，不能用本机时钟、等待时长或查不到交易独立判定。
  仍有效的签名、缺少广播哈希、终态冲突、区块/账户变化、范围不完整全部停止。
- 审计后只追加 `bridge_tx_expired_slot`。旧签名字节、operationId、哈希、状态保持；`next_isn` 不降低。
  新请求在共享账户锁内一次性领取预留，并再次检查见证区块和旧签名有效期。签名或数据库失败回滚领取；未知广播仍保留原签名。
- 同一序号可以对应不同时间的历史签名，因此提交表的永久唯一序号索引改为普通索引；账户锁、高水位和唯一未领取预留约束
  保证新的有效分配不冲突。旧 operationId 始终返回旧哈希，不为旧业务创建第二笔交易。
- 发布时暂停所有写入口并备份，先执行 `dioxide_tx_expiry_migration.sql` 与更新的建表 SQL，再升级 Plugin Server、
  两个 Dioxide 插件及 Python 模块，最后显式创建已核验的预留。数据库不可用或新表缺失时停止分配。
- 回滚先关闭新入口、保留/对账新签名及预留领取记录，再恢复对应组件；不能恢复数据库快照或重建永久唯一序号索引。
  旧版本不能领取过期预留，因此回滚后不得宣称新交易已恢复。

本轮真实边界：旧 427–440 的签名已过期且不可查询，用户要求只保障新业务。实验发送的新 441 虽然返回哈希，
仍为 TXN_READY，证明“只校正节点观察值、从高水位继续”不足以恢复执行。新流程不补发这些历史交易，不用空交易填洞。
