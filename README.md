...
本分支基于[AntChainOpenLabs
AntChainBridge](https://github.com/AntChainOpenLabs/AntChainBridge)开发，添加了dioxide插件，完成了如下内容：
- **dioxide链上插件的开发**：包括跨链系统合约（AM合约和SDP合约）的gcl版本，负责链上跨链消息的可信收发处理。**需要注意的是，目前仅支持SDPv1版本的消息。**
- **dioxide链下插件的开发**：包括一系列完成中继和异构链之间的信息交互处理的接口的实现。通常由链下插件来发起对链上跨链系统合约的部署和调用。**需要注意的是，目前dioxide插件不支持基于委员会PTC的跨链消息验证功能，仅支持跨链消息传输功能。**

目前本系统能够完成dioxide链与ethereum等链的跨链互通，具体的部署和操作流程详见语雀文档。
